...
// derivable and because getting it wrong is silent: hardcoding `flow0()` through
// `flow3()` works for a four-flow program and quietly drops the last flow of a
// five-flow one. `koruc` emits one `flowN` per top-level flow in source order,
// and `@typeInfo` already knows how many there are.
